...
COMMON_PORTS = [
    ...
    465, 587, 993, 995, 1433, 1521, 2022, 2080, 2082, 2083, 2086,
    2087, 3000, 3001, 3306, 3389, 5432, 5900, 6379, 8000, 8080,
...
PORT_SERVICES = {
    ...
    2022: "SSH-alt", 2080: "HTTP-alt", 2082: "cPanel", 2083: "cPanel SSL",
    2086: "WHM", 2087: "WHM SSL", 3000: "Grafana/Node",
...
@app.get("/port-scan")
async def port_scan(host: str, ports: str = "common"):
    ...
    if ports == "common":
        ...
        try:
            ...
            if not all(1 <= p <= 65535 for p in custom_ports):
                raise HTTPException(status_code=400, detail="Ports must be between 1 and 65535")
            port_list = ",".join(str(p) for p in custom_ports)
        except ValueError:
            raise HTTPException(status_code=400, detail="Invalid port format")

    try:
        nm.scan(hosts=host, ports=port_list, arguments="-T4 --open -sV")
    except Exception as e:
    ...
    if host in nm.all_hosts():
        for proto in nm[host].all_protocols():
            for port in sorted(nm[host][proto].keys()):
                ...
                service = nm[host][proto][port].get("name", PORT_SERVICES.get(port, "unknown"))
                ...
                results.append({
                    ...
                    "product": product,
                    "version": version,
                    "description": PORT_SERVICES.get(port, service)
                })

    return {
        "host": host,
        "ports_scanned": len(COMMON_PORTS) if ports == "common" else len(port_list.split(",")),
